...
const ProfitsCard: FC = () => {
  const supabase = useSupabaseClient()

  const loadProfits = async (): Promise<number | undefined> => {
    const { data: orders } = await supabase.from('orders_detail')
      .select('quantity, price, products(cost)')
    const { data: sales } = await supabase.from('sales_detail')
      .select('quantity, price, products(cost)')

    if (orders !== undefined && orders !== null && sales !== undefined && sales !== null) {
      const totalOrders = orders.reduce((acc, current) => {
        // @ts-expect-error
        const cost = current.products.cost
      ...
      const totalSales = sales.reduce((acc, current) => {
        // @ts-expect-error
        const cost = current.products.cost
        const price = current.price
        const quantity = current.quantity
  ...
  const { data: total, isLoading } = useQuery(['profits'], loadProfits)
